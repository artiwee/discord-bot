...
import { evaluate as _ } from "ghom-eval";
import { code } from "./utils.js";
export async function evaluate(input, options, embedPresets = {
    ...
    if (options?.muted) {
        return embedPresets.success.setDescription(`Successfully evaluated in ${evaluated.duration}ms`);
    }
    else {
        const embed = embedPresets[evaluated.failed ? "error" : "success"]
            .setTitle(`Result ${evaluated.failed ? "(failed)" : ""}`)
            .setDescription(await code.stringify({
